...
object Evaluator {
  ...
  private def letEnv(defs: List[MalExpr], outer: Environment): Either[MalError, Environment] = {
    ...
    val definitions = liftError(defs.grouped(2).map {
  ...
  private def liftError[E, V](s: Seq[Either[E, V]]): Either[E, List[V]] = {
    s.foldRight(Right(Nil): Either[E, List[V]]) {
      (either, acc) => for (xs <- acc; x <- either) yield x :: xs
    }
  }

  private def liftError[E, K, V](m: Map[K, Either[E, V]]): Either[E, Map[K, V]] = {
    m.foldRight(Right(Map()): Either[E, Map[K, V]]) {
      case ((key, either), acc) => for (m <- acc; value <- either) yield m + (key -> value)
    }
  }

  def evaluate_ast(ast: MalExpr, env: Environment): Either[MalError, MalExpr] = ast match {
    case sym: MSymbol => env.get(sym)
    case MList(elems) => elems match {
      case _ => liftError(elems.map(evaluate(_, env))).map { items => MList(items) }
    }
    case MVector(elems) => liftError(elems.map(evaluate(_, env))).map { items => MVector(items) }
    case MMap(elems) => liftError(elems.mapValues(evaluate(_, env))).map { m => MMap(m) }
    case _ => Right(ast)
  }
}
